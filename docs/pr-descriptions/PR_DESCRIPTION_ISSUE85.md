...
- `app/components/scenario/ShareButtons.tsx`を新規作成
  ...
  - 各SNSのインテントURLを動的に生成
  - 投稿内容にステージ名、キケン度、シナリオコードを自動で含める
...
sequenceDiagram
    ...
    ScenarioDetailClient->>ShareButtons: シナリオ情報を渡す<br/>(code, stageName, dangerRate)
...
- テキスト: 「ステージ: {StageName} / キケン度: {DangerRate}% / シナリオコード: {ScenarioCode}」
- ハッシュタグ: `サーモンランNW,SalmonRunScenarioHub,サーモンランシナリオhub`

### BlueSky
- URL: `https://bsky.app/intent/compose?text={TEXT}%20{URL}`
- テキスト: 「ステージ: {StageName} / キケン度: {DangerRate}% / シナリオコード: {ScenarioCode} {URL}」
...
- [x] 投稿内容にステージ名、キケン度、シナリオコードが含まれることを確認
