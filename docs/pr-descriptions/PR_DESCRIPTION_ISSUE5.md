...
加えて、武器マスタデータに「緑ランダム」「黄ランダム」を追加し、画像解析プロンプトを更新しました。また、ESLintエラーの修正とテストの更新も実施しました。

## 実施内容

### 0. 武器マスタデータの拡張

- **緑ランダム・黄ランダムの追加**: 武器マスタデータ（`m_weapons`）に「緑ランダム」「黄ランダム」を追加
- **画像解析プロンプトの更新**: 緑色の「?」マークを「緑ランダム」、黄色の「?」マークを「黄ランダム」として認識するようプロンプトを更新
- **データベースマイグレーション**: 既存のマイグレーション（`003_import_master_data.sql`）でインポートされるシードデータ（`supabase/seed/m_weapons.json`）を更新
...
#### GET /api/weapons
- **機能**: 武器マスタ一覧を取得
- **レスポンス**: 武器ID、名前、アイコンURLのリスト
- **ソート**: 名前順
- **用途**: 武器フィルターのアイコン表示に使用

### 5. GET /api/scenarios エンドポイントの実装
...
- `app/api/stages/route.ts` - ステージマスタ一覧取得APIエンドポイント
- `app/api/weapons/route.ts` - 武器マスタ一覧取得APIエンドポイント（`icon_url`含む）
- `app/analyze/page.tsx` - 画像解析ページ
- `PR_DESCRIPTION_ISSUE5.md` - 本PR説明文

### 更新
- `app/page.tsx` - シナリオ一覧ページに変更（フィルター機能追加）
- `app/api/scenarios/route.ts` - GETエンドポイントを追加（フィルター対応）、ESLintエラー修正（`console.log` → `console.warn`、未使用インポート削除）
- `app/api/weapons/route.ts` - `icon_url`フィールドをレスポンスに追加（武器フィルターのアイコン表示に対応）
- `app/__tests__/page.test.tsx` - 新しいページ構造に合わせてテストを更新
- `app/components/ImageAnalyzer.tsx` - プレビューURLのクリーンアップとAbortControllerの実装、ESLintエラー修正（`console.log` → `console.warn`、未使用変数削除）
- `supabase/seed/m_weapons.json` - 武器マスタデータに「緑ランダム」「黄ランダム」を追加
- `lib/ai/prompt.ts` - 画像解析プロンプトにランダム武器の認識ルールを追加
...
### ESLintエラーの修正
- `console.log`を`console.warn`に変更（ESLintの`no-console`ルールに準拠）
- 未使用のインポートと変数を削除（`no-unused-vars`ルールに準拠）
- すべてのESLintエラーを解消

### リソース管理の改善
- `ImageAnalyzer`コンポーネントで`URL.createObjectURL`で作成したプレビューURLを適切にクリーンアップ
- `AbortController`を使用してコンポーネントアンマウント時に進行中のfetchリクエストをキャンセル
- メモリリークの防止
...
- 緑ランダム・黄ランダムは、画像上ではそれぞれ緑色・黄色の「?」マークとして表示されます
